...
-- must return the same result as the legacy searcher. Each assertion below compares the forced path
-- against the legacy path on the same input and prints 1 when they agree. The cases exercise
-- malformed UTF-8 and length-growing lowercase mappings, where a naive fold diverges from the legacy
-- searcher (which advances over invalid input by UTF8::seqLength).
...
-- Truncated lead byte against a NUL needle: legacy skips past the trailing terminator.
