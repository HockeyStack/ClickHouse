...
Force use of the daachorse (Aho-Corasick) algorithm for the `multiSearchAny*` family of functions (all case-sensitivity and UTF-8 variants), even when the pattern count is below the automatic threshold (255). Normally daachorse is only used when there are more than 255 patterns. This setting has no effect on other multi-search functions such as `multiSearchFirstIndex*`, `multiSearchFirstPosition*`, or `multiMatch*`. It is useful for testing or when daachorse performs better for your specific workload.
)", 0) \
