...
/// Replaces the process-wide direct-mapped cache with `slots` empty slots. Zero disables caching.
...
///
/// The cache is direct-mapped: a collision replaces the resident entry, following the same pattern
/// as Hyperscan's GlobalCacheTable in Regexps.h.
