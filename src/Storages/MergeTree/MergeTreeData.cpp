...
void MergeTreeData::checkAlterIsPossible(const AlterCommands & commands, ContextPtr local_context) const
{
    ...
    auto get_materialized_dependencies_for_clear = [&]() -> const MaterializedColumnDependencies &
    {
        ...
        {
            NamesAndTypesList all_columns_with_ephemeral = old_metadata.getColumns().getAllPhysical();
            NameSet ephemeral_columns;
            for (const auto & column : old_metadata.getColumns().getEphemeral())
            {
                ephemeral_columns.insert(column.name);
                all_columns_with_ephemeral.push_back(column);
            }

            MaterializedColumnDependencyOptions options;
            options.ephemeral_columns = &ephemeral_columns;

            materialized_dependencies_for_clear = buildMaterializedColumnDependencies(
                old_metadata.getColumns(),
